  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `https://api.unsplash.com/photos?page=${page}&per_page=20&client_id=dthP8VNzMTvgPPCHRVAInVjlov43oenGQx8UkNn2VqE`
      );
...
export function useUnsplashSearchImages(searchValue) {
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => {
      const response = await fetch(
        `https://api.unsplash.com/search/photos?&query=${searchValue}&page=${pageParam}&per_page=20&client_id=I52UPWjSrVe5VkIVEs3H9VIy4VX9XFBi9XQ5kf8VkFg`
      );
